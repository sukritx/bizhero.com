...
const icons = [
  ...
  (<svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="5" r="2" /><path d="M10 22v-4H6.5a2.5 2.5 0 0 1 0-5H10" /><path d="M14 22v-4h3.5a2.5 2.5 0 0 0 0-5H14" /><path d="M12 7v6" /></svg>),
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mx-auto mb-16 max-w-[700px] text-center">
          ...
          <h2 className="mb-4 text-3xl font-bold leading-tight text-gray-900 sm:text-4xl">
            {t("ourServices.headline")}
          </h2>
          <p className="text-lg text-gray-600">{t("ourServices.subtitle")}</p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
          {productCategories.map((category, i) => (
            <div key={i} className="group rounded-xl border border-gray-200 bg-white p-8 shadow-sm transition hover:shadow-lg">
              ...
              <h3 className="mb-3 text-xl font-bold text-gray-900">{t(category.titleKey)}</h3>
              <p className="mb-4 text-base leading-relaxed text-gray-600">{t(category.descKey)}</p>
              <Link href={category.href} className="inline-flex items-center text-sm font-semibold text-primary transition hover:text-primary/80">
                {t("ourServices.viewProducts")}
